...
log_dir=./pyramid_moe_top1_4_8_stage1_logs
save_dir=./pyramid_moe_top1_4_8_stage1_checkpoints
...
restore_file=../../checkpoints/nah.pt
selected_cols=0,4,2

task=caption
arch=ofa_shared_moe_base
...
for max_epoch in {15,}; do
  ...
  for warmup_ratio in {0.06,}; do
    ...
    for drop_worst_after in {2500,}; do
      ...
      CUDA_VISIBLE_DEVICES=3 python3 -m torch.distributed.launch --nproc_per_node=1 --master_port=${MASTER_PORT} ../../train.py \
          $data \
          ...
          --scale-heads \
          --moe-expert-count=4 \
          --moe-encoder-expert-count=4 \
          --moe-decoder-expert-count=8 \
          --moe-freq=0 \
          --encoder-moe-freq=0 \
          --decoder-moe-freq=6 \
          --moe-gating-use-fp32 \
